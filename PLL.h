...
#define PSYSDIV 26 
//fVC0 = (fXTAL/(Q + 1)/(N + 1))*(MINT + (MFRAC/1,024)) = 480MHz
//SYSCLK = fVCO / (PSYSDIV + 1) 
//Q = 0, N = 4, MINT = 96, fXTAL = 25000000, MFRAC = 0
//If SYSCLK has to be 18MHz: 18(10^6) = 480(10^6)/(PSYSDIV+1)
//Therefore PSYSDIV+1 = 26.667 
//Since we must set PSYSDIV = 26 (since it must be an integer and we round up to ensure the clock frequency does not exceed 18MHz)
//The bus frequency will therefore be 17.778 MHz
//NOTE: You will have to change the SysTick function value to change the delay accordingly 
